...
p <- ggplot(final_dt, aes(x = clean_name, y = fit_score, fill = type)) +
  ...
  labs(
    title = "Nemo Temperature Sweep: First-Order Fit",
    subtitle = "Comparing marginal response distributions (1 - Mean TVD) across temperatures.\nA higher score indicates better alignment with the human GSS 2024 baseline.",
...
# Save
viz_file <- file.path(BASE_VIZ_DIR, "nemo_temperature_fit_sweep.pdf")
...
# Save stats
stats_file <- file.path(BASE_OUT_DIR, "nemo_temp_sweep_stats.csv")
